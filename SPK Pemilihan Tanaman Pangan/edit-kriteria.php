...
if(isset($_POST['submit'])):	
	...
	if(empty($errors)):
		...
		$prepare_query = 'UPDATE kriteria SET nama = :nama, type = :type, bobot = :bobot, ada_pilihan = :jenis_nilai WHERE id_kriteria = :id_kriteria';
		$data = array(
			'nama' => $nama,
			'type' => $type,
			'bobot' => $bobot,
		...
		if(!empty($pilihan)): foreach($pilihan as $pil):
			...
			if($id_pil_kriteria && $nama != '' && ($nilai >= 0)):				
				...
				$prepare_query = 'UPDATE pilihan_kriteria SET nama = :nama, id_kriteria = :id_kriteria, nilai = :nilai WHERE id_pil_kriteria = :id_pil_kriteria';
				$data = array(
					'nama' => $nama,
					'id_kriteria' => $id_kriteria,
					'nilai' => $nilai,
			...
			elseif(($nama != '') && ($nilai >= 0)):
				...
				$prepare_query = 'INSERT INTO pilihan_kriteria (nama, id_kriteria, nilai) VALUES  (:nama, :id_kriteria, :nilai)';
				$data = array(
					'nama' => $nama,
					'id_kriteria' => $id_kriteria,
					'nilai' => $nilai,
					...
					<div class="field-wrap list-var clearfix <?php echo $the_class; ?>">					
						...
						<table id="pilihan-var" class="pure-table pure-table-striped">
							...
							<tbody>
								...
								<?php
								$query = $pdo->prepare('SELECT * FROM pilihan_kriteria WHERE id_kriteria = :id_kriteria ');			
								$query->execute(array(
									'id_kriteria' => $id_kriteria
								));
