...
$id_administrasi_kel_uts = $_GET['id_administrasi_kel_uts'];
...
$path = "../Data_Folder/$nama_ta/$nama_jurusan/$nama_kelas/$nama_mapel/$sesudah";
...
$query = mysqli_query($koneksi, "DELETE FROM administrasi_kel_uts WHERE id_administrasi_kel_uts = $id_administrasi_kel_uts");
if($query){
	$query_guru1=mysqli_query($koneksi, "SELECT * FROM guru WHERE id_guru ='$id_guru'");
	$data_guru1 = mysqli_fetch_array($query_guru1);
	$value_uts=$data_guru1['value_uts'];
	$value_uts1=$value_uts-1;
	$query_value_uts= mysqli_query($koneksi, "UPDATE `guru` SET `value_uts`='$value_uts1' WHERE id_guru = '$id_guru'") or die(mysqli_error($connect));
	$query_tampil = mysqli_query($koneksi, "SELECT * FROM guru WHERE id_guru ='$id_guru'");
	while ($data_guru = mysqli_fetch_array($query_tampil)) 
	{
		if($data_guru['jumlah']>$data_guru['value_uts1'])
		{
