    ...
    <table class="table table-striped">
      ...
      if($sql->num_rows > 0)
      {
        ...
        while($row = $sql->fetch_assoc())
        {
          ...
          <tr>
            <td>'.$no.'</td>
            <td>'.$row['perhitungan_minggu_efektif'].' <br> <a href="terupload/'.$row['perhitungan_minggu_efektif'].'" class="btn btn-primary btn-sm">Download</a>
            </td>

            <td>'.$row['program_tahunan'].' <br> <a href="Daftar_Folder/'.$row['perhitungan_minggu_efektif'].'" class="btn btn-primary btn-sm">Download</a>
            </td>

            <td>'.$row['program_tahunan'].' <br> <a href="Daftar_Folder/'.$nama_tahun_ajaran.'/'.$nama_jurusan.'/'.$nama_kelas.'/'.$nama_mapel.'/'.$row['program_tahunan'].'" class="btn btn-primary btn-sm">Download</a>
            </td>
            <td>'.$row['program_semester'].' <br> <a href="terupload/'.$row['program_semester'].'" class="btn btn-primary btn-sm">Download</a>
            </td>
            <td>'.$row['silabus'].' <br> <a href="terupload/'.$row['silabus'].'" class="btn btn-primary btn-sm">Download</a>
            </td>
            <td>'.$row['analisis_kkm'].' <br> <a href="terupload/'.$row['analisis_kkm'].'" class="btn btn-primary btn-sm">Download</a>
            </td>
            <td>'.$row['rpp'].' <br> <a href="terupload/'.$row['rpp'].'" class="btn btn-primary btn-sm">Download</a></td>
          </tr>
          ';
          $no++;
        }
